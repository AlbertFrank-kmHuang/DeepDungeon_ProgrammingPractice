...
public:
    ViperFang(const std::string& name="毒蛇之牙", Rarity rarity=Rarity::Epic):
        ...
        critdamageBonus=0.5;
        critrateBonus=0.2;
        description="浸透毒液的匕首，来自毒蛇的牙齿，增加0.5的暴击伤害，0.2的暴击率，攻击时让敌人陷入中毒，持续一回合";
        Skill* skill=new buffSkill(1,"中毒",Status::Poisoned,1,0.15,"攻击后，使目标陷入持续1回合的中毒状态");
        specialEffects.push_back(skill);
    }
};
...
public:
    LightningRod(const std::string& name="雷光魔杖", Rarity rarity=Rarity::Epic):
        ...
        Skill* skill=new buffSkill(1,"麻痹",Status::Paralyzed,1,0,"攻击后，使敌人陷入1回合的麻痹状态");
        specialEffects.push_back(skill);
    }
};
...
public:
     GuardianAegis(const std::string& name="守护者之盾", Rarity rarity=Rarity::Epic):
        ...
        Skill* skill=new buffSkill(1,"反击",Status::Defend, 1, 0.5,"进入防御姿态，减伤50%，受到攻击后反击");
        specialEffects.push_back(skill);
    }
};
...
public:
    Frostwhisper(const std::string& name="霜语者", Rarity rarity=Rarity::Epic):
        ...
        critdamageBonus=0.5;
        critrateBonus=0.2;
        description="冰霜为箭，增加0.5的暴击伤害，0.2的暴击率，攻击后让敌人陷入麻痹，持续一回合";
        Skill* skill=new buffSkill(1,"麻痹",Status::Paralyzed,1,0,"攻击后，使敌人陷入1回合的麻痹状态");
        specialEffects.push_back(skill);
    }
};
...
public:
    Ragnarok(const std::string& name="诸神黄昏", Rarity rarity=Rarity::Legendary):
        ...
        Skill* skill=new buffSkill(1,"吸血",Status::LifeSteal,1,0.3,"攻击后，附加一回合的吸血效果");
        specialEffects.push_back(skill);
    }
};
...
public:
    SwordofDestiny(const std::string& name="命运之剑", Rarity rarity=Rarity::Legendary):
        ...
        Skill* skill=new buffSkill(1,"护盾",Status::shieldAdd,1,0.3,"攻击后附加护盾（生命值30%），护盾持续1个回合");
        specialEffects.push_back(skill);
    }
};
...
public:
    Fatepiercer(const std::string& name="命运穿刺", Rarity rarity=Rarity::Legendary):
        ...
        Skill* skill=new buffSkill(1,"穿透",Status::Penetration,1,0,"攻击后附加一回合的穿透");
        specialEffects.push_back(skill);
    }
};
...
public:
    Worldender(const std::string& name="灭世者", Rarity rarity=Rarity::Legendary):
        ...
        Skill* skill=new buffSkill(1,"速切",Status::fastchange,1,0,"释放技能后可以直接切人而不消耗回合");
        specialEffects.push_back(skill);
    }
};
...
public:
    SpindleofFate(const std::string& name="命运纺锤", Rarity rarity=Rarity::Legendary):
        Weapon(name,rarity){
        attackBonus=0.3;
        critdamageBonus=0.5;
        critrateBonus=0.3;
        description="独自编织命运，中幅提升攻击力（1.3倍），无法增加弓箭手后台攻击伤害，增加0.5的暴击伤害，0.3的暴击率，释放技能不再消耗法力";
        Skill* skill=new buffSkill(1,"无消耗",Status::fixed,100000000,0,"释放技能不再消耗法力");
        specialEffects.push_back(skill);
    }
};
...
public:
    AbsoluteDefense(const std::string& name="绝对防御", Rarity rarity=Rarity::Legendary):
        ...
        defenseBonus=1.0;
        missrateBonus=0.5;
        description="无坚不摧，大幅增加防御力（2倍），增加0.5的闪避率，攻击后为自身附加护盾，持续一回合";
        Skill* skill=new buffSkill(1,"护盾",Status::shieldAdd,1,0.3,"攻击后附加护盾（生命值30%），护盾持续1个回合");
        specialEffects.push_back(skill);
    }
};
