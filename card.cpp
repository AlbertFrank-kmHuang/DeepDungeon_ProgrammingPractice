...
bool BuffCard::use(Character* target) {
    ...
    case Status::critrateAdd:
        ...
        target->setcritrateadd(buffValue);
        break;
    case Status::critdamageAdd:
        buffmount=int(buffValue*100);
        target->setcritdamageadd(buffValue);
        break;
    case Status::missrateAdd:
        buffmount=int(buffValue*100);
        target->setmissrateadd(buffValue);
        break;
...
bool EventCard::use(Character* target) {

    // 生成随机奖励
    std::uniform_int_distribution<int> rewardDist(0, 3);
    int rewardChoice = rewardDist(rng);
    ...
    case EventType::Treasure:
        ...
        case 0: // 金币
            reward.type = RewardType::Gold;
            reward.value = std::uniform_int_distribution<int>(50, 200)(rng);
            break;
        case 1: // 全队治疗
            reward.type = RewardType::HealAll;
            reward.value = std::uniform_int_distribution<int>(20, 50)(rng);
            break;
        case 2: // 角色升星
        ...
        case 3: // 装备
            ...
            std::string weaponName = epicWeaponList[weaponDist(rng)];
    ...
    case EventType::Challenge:
        ...
        case 1:{ // 传奇装备
            ...
            std::string weaponName = legendaryWeaponList[weaponDist(rng)];
            reward.weapon = createWeaponByName(weaponName);
            break;
        }
        case 2:{ // 大量金币
            reward.type = RewardType::Gold;
            reward.value = std::uniform_int_distribution<int>(100, 300)(rng);
            break;
        }
        case 3:{ // 大量治疗
            reward.type = RewardType::HealAll;
            reward.value = std::uniform_int_distribution<int>(50, 100)(rng);
            break;
        }
        }
        break;
    ...
    default:
        // 默认给金币
        reward.type = RewardType::Gold;
        reward.value = std::uniform_int_distribution<int>(10, 50)(rng);
        break;
    }
...
public:
    FireballCard(Character* target = nullptr)
        : AttackCard("火球术", "对目标造成30点火焰伤害", target, 30, Status::Normal, 0) {}
};
...
public:
    IceSpikeCard(Character* target = nullptr)
        : AttackCard("冰锥术", "对目标造成20点伤害并使其麻痹1回合",
...
public:
    LightningChainCard(Character* target = nullptr)
        : AttackCard("闪电链", "对目标造成15点伤害，麻痹两回合",
...
public:
    StrengthBlessingCard(Character* target = nullptr)
        : BuffCard("力量祝福", "使目标攻击力提升30%，持续3回合",
...
public:
    GuardianLightCard(Character* target = nullptr)
        : BuffCard("守护之光", "使目标获得50%生命值的护盾，持续2回合",
...
public:
    SwiftWindCard(Character* target = nullptr)
        : BuffCard("迅捷之风", "使目标闪避率提升30%，持续2回合",
...
public:
    MysteryTreasureCard(Character* target = nullptr)
        : EventCard("神秘宝箱", "打开一个神秘宝箱，获得随机奖励",
